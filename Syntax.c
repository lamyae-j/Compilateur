...
bool verifyToken(token_code code_to_test, int error){
    if (error)
        printf("%s", token_code[code_to_test]);
        exit(1);

    return (currentToken = getToken()).code != code_to_test;
}
